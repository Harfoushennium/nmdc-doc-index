...
Table-heavy review sheets require a **Dynamic Live Filter** based on the owner's original reference module. The final owner UX must provide a **real editable search input** that can be clicked and typed into normally, update results on each keystroke, let the user choose the target column, and retain the reference search features such as normal partial matching, excluded words, exact phrases and wildcard matching. The currently rejected fake-shape/keyboard-capture implementation is not an accepted release design; see `ANTIGRAVITY_HANDOFF.md`.
