...
def write_outputs(workbooks:List[Dict[str,object]],class_rows:List[Dict[str,object]],exact_groups:List[Dict[str,object]],version_groups:List[Dict[str,object]],output_dir:Path)->None:
    ...
    write_text_lf(output_dir/"workbook_profiles.json",json.dumps(sorted(workbooks,key=lambda w:str(w["relative_path"]).lower()),indent=2,ensure_ascii=False,sort_keys=True)+"\n")
    ...
    write_text_lf(output_dir/"source_selection_report.md","\n".join(lines))
