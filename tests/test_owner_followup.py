...
class OwnerFollowupTests(unittest.TestCase):
    ...
    def test_workbook_guidance_explains_inputs_outputs_and_pending_update(self):
        ...
        self.assertIn("REVIEW + SOURCE SELECTION", startup)
        self.assertIn('Case "INCLUDE IN INDEX?"', startup)
        self.assertIn('Case "OWNER NOTE"', startup)
    ...
    def test_table_presentation_is_professional_unfilled_and_aligned(self):
        startup = (ROOT / "excel" / "vba" / "modNMDC_Startup.bas").read_text(encoding="utf-8")
        refresh = (ROOT / "excel" / "vba" / "modNMDC_Refresh.bas").read_text(encoding="utf-8")
        self.assertIn('table.TableStyle = "TableStyleMedium2"', startup)
        self.assertIn("table.ShowTableStyleRowStripes = False", startup)
        self.assertIn("table.ShowTableStyleColumnStripes = False", startup)
        self.assertIn('table.Range.Font.Name = "Aptos"', startup)
        self.assertIn(".Interior.Pattern = xlNone", startup)
        self.assertIn(".Font.ColorIndex = xlAutomatic", startup)
        self.assertIn("Private Sub NMDC_ApplyColumnAlignment", startup)
        self.assertIn("Private Sub NMDC_AdjustTableRows", startup)
        self.assertIn("table.DataBodyRange.Rows.AutoFit", startup)
        self.assertIn("If oneRow.RowHeight > 60 Then oneRow.RowHeight = 60", startup)
        self.assertIn("NMDC_SetColumnWidth column", startup)
        self.assertIn("NMDC_ApplyTableGuidance table", refresh)
        self.assertNotIn("decisionRange.Interior.Color =", refresh)
        self.assertNotIn("commentRange.Interior.Color =", refresh)
        self.assertNotIn("statusRange.Interior.Color =", refresh)

    def test_refresh_resets_filters_and_sort_before_replacing_rows(self):
        refresh = (ROOT / "excel" / "vba" / "modNMDC_Refresh.bas").read_text(encoding="utf-8")
        self.assertIn("Private Sub NMDC_ResetTableViewForRefresh", refresh)
        self.assertIn("If table.Parent.FilterMode Then table.Parent.ShowAllData", refresh)
        self.assertIn("table.Sort.SortFields.Clear", refresh)
        self.assertIn("table.ShowAutoFilter = True", refresh)
        reset_position = refresh.index("NMDC_ResetTableViewForRefresh table")
        clear_position = refresh.index("table.DataBodyRange.ClearContents")
        resize_position = refresh.index("table.Resize targetRange")
        self.assertLess(reset_position, clear_position)
        self.assertLess(reset_position, resize_position)

    def test_rules_sheet_has_non_coder_simple_editor(self):
        startup = (ROOT / "excel" / "vba" / "modNMDC_Startup.bas").read_text(encoding="utf-8")
        self.assertIn("SIMPLE RULE EDITOR", startup)
        self.assertIn("Public Sub NMDC_ConfigureRulesUserExperience", startup)
        self.assertIn("Public Sub NMDC_AddSimpleRule", startup)
        self.assertIn("Public Sub NMDC_ToggleRulesAdvancedColumns", startup)
        self.assertIn('"ANY,METHODS,TECH"', startup)
        self.assertIn('"FILE,WORKSHEET,SECTION,HEADER,DOC_NUMBER,TITLE"', startup)
        self.assertIn('"CONTAINS,EXACT,STARTS_WITH,ENDS_WITH"', startup)
        self.assertNotIn('"CONTAINS,EXACT,FUZZY,REGEX"', startup)
        self.assertIn('"YES,NO"', startup)
        self.assertIn('button.TextFrame.Characters.Text = "Add Simple Rule"', startup)
        self.assertIn('button.TextFrame.Characters.Text = "Show / Hide Advanced"', startup)
        self.assertIn('names = Array("Rule_ID", "Priority", "Exclude_Words", "Path_Qualifier"', startup)
        self.assertIn('NMDC_SetRuleCell table, newRow, "Match_Type", "CONTAINS"', startup)
        self.assertIn('NMDC_SetRuleCell table, newRow, "Min_Confidence", 0.9', startup)
        self.assertIn("CONTAINS is recommended", startup)
        self.assertIn("Technical legacy rule types stay outside the normal owner workflow", startup)
        self.assertIn("If isRules Then", startup)
